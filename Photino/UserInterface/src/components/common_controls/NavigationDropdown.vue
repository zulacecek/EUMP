...
import { ref } from 'vue';
import { DropdownOptions } from '../../structs/missionStructs';
import { NavigationDropdownViewModel } from '../../structs/controlsStructs';

const props = defineProps<{viewModel: NavigationDropdownViewModel }>();
...
function getOptionsStyle() {
    return `max-height: ${props.viewModel.dropdownMaxHeight}px; overflow-y: hidden;`;
}
...
<template>
    <div class="custom-dropdown">
        ...
        <div class="big-custom-button thin-golden-border" :class="{ 'selected': viewModel.isSelected }" style="min-height: 30px;" @click="openDropdown()"> {{ props.viewModel.selectedOption?.Label }} </div>
